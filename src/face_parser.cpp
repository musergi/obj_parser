...
FaceParser::FaceParser(const std::string &face_string) : face_string(face_string) {
    point_offset = face_string.find(' ') + 1;
    point_length = face_string.find(' ', point_offset) - point_offset;
}

bool FaceParser::hasPoint() const { return point_offset < face_string.size(); }

const Point FaceParser::nextPoint() {
    unsigned int v, vt = 0, vn = 0;
    const Point point = parsePoint(face_string.substr(point_offset, point_length));
    movePointers();
    return point;
}
